...
Std_ReturnType SPI_Send_Byte(const SPI_Config *Config, const uint8 _data){
    ...
    if(NULL == Config){
        ...
        SSPBUF = _data;
        //while(PIR1bits.SSPIF); // block till  it`s waiting to transmit/receive <0>
        while(SSPSTATbits.BF); /* block till SSPBUF is empty */
        PIR1bits.SSPIF = 0;
    }
...
Std_ReturnType SPI_Read_Byte(const SPI_Config *Config, uint8 *_data){
    ...
    if((NULL == Config) || (NULL == _data)){
        ...
        while(SSPSTATbits.BF == 0); /* Receive not complete, SSPBUF is empty  block till SSPBUF is full */
        *_data = SSPBUF;
    }
...
Std_ReturnType SPI_Send_Byte_NonBlocking(const SPI_Config *Config, const uint8 _data){
    ...
    if(NULL == Config){
        ...
        __delay_us(10);
        if(!PIR1bits.SSPIF){ // if it`s waiting to transmit/receive
        SSPBUF = _data;
        }
        else{
            ret = E_NOT_OK;
...
Std_ReturnType SPI_Read_Byte_NonBlocking(const SPI_Config *Config, uint8 *_data){
    ...
    if(NULL == Config){
        ...
        if(PIR1bits.SSPIF){ // if transmission is complete
        *_data = SSPBUF;
        }
        else{
            ret = E_NOT_OK;
