...
Std_ReturnType EUSART_ASYNC_WriteStringBlocking(uint8 *_data) {
  Std_ReturnType ret = E_OK;
  while (*_data != '\0') {
    ret = EUSART_ASYNC_WriteByteBlocking(*_data);
    *_data++;
  }
  return ret;
}
...
Std_ReturnType EUSART_ASYNC_WriteStringNonBlocking(uint8 *_data) {
  Std_ReturnType ret = E_OK;
  while (*_data != '\0') {
    ret = EUSART_ASYNC_WriteByteNonBlocking(*_data);
    _data++;
    __delay_ms(5);
  }
  return ret;
...
static void EUSART_ASYNC_TX_Init(const usart_t *_eusart) {
  if (EUSART_ASYNCHRONOUS_TX_ENABLE == _eusart->usart_tx_cfg.usart_tx_enable) {
    ...
    if (EUSART_ASYNCHRONOUS_INTERRUPT_TX_ENABLE ==
        _eusart->usart_tx_cfg.usart_tx_interrupt_enable) {
      ...
      if (INTERRUPT_HIGH_PRIORITY ==
          _eusart->usart_tx_cfg.usart_tx_int_priority) {
...
#endif
#endif
    } else if (EUSART_ASYNCHRONOUS_INTERRUPT_TX_DISABLE ==
               _eusart->usart_tx_cfg.usart_tx_interrupt_enable) {
    ...
    if (EUSART_ASYNCHRONOUS_9Bit_TX_ENABLE ==
        _eusart->usart_tx_cfg.usart_tx_9bit_enable) {
      TXSTAbits.TX9 = EUSART_ASYNCHRONOUS_9Bit_TX_ENABLE;
    } else if (EUSART_ASYNCHRONOUS_9Bit_TX_DISABLE ==
               _eusart->usart_tx_cfg.usart_tx_9bit_enable) {
...
static void EUSART_ASYNC_RX_Init(const usart_t *_eusart) {
  if (EUSART_ASYNCHRONOUS_RX_ENABLE == _eusart->usart_rx_cfg.usart_rx_enable) {
    ...
    if (EUSART_ASYNCHRONOUS_INTERRUPT_RX_ENABLE ==
        _eusart->usart_rx_cfg.usart_rx_interrupt_enable) {
      ...
      INTERRUPT_PriorityLevelsEnable();
      if (INTERRUPT_HIGH_PRIORITY ==
          _eusart->usart_rx_cfg.usart_rx_int_priority) {
        /* Enables all high-priority interrupts */
        INTERRUPT_GlobalInterruptHighEnable();
...
#endif
#endif
    } else if (EUSART_ASYNCHRONOUS_INTERRUPT_RX_DISABLE ==
               _eusart->usart_rx_cfg.usart_rx_interrupt_enable) {
    ...
    if (EUSART_ASYNCHRONOUS_9Bit_RX_ENABLE ==
        _eusart->usart_rx_cfg.usart_rx_9bit_enable) {
      RCSTAbits.RX9 = EUSART_ASYNCHRONOUS_9Bit_RX_ENABLE;
    } else if (EUSART_ASYNCHRONOUS_9Bit_RX_DISABLE ==
               _eusart->usart_rx_cfg.usart_rx_9bit_enable) {
...
void EUSART_RX_ISR(void) {
  ...
  if (EUSART_FramingErrorHandler && RCSTAbits.FERR) {
    EUSART_FramingErrorHandler();
  } else { /* Nothing */
  }
  if (EUSART_OverrunErrorHandler && RCSTAbits.OERR) {
